...
public final class RoyaleRenderer: DialRenderer {
    ...
    private func installLayers() {
        ...
        // hour hand → minute hand → seconds tick (top so it's always visible
        // against the hands). All sit on lcdLayer, visible through the
        // subdial cutout in the faceplate above.
    ...
    private func layoutLCDContent(cutouts: CutoutGeometry) {
        ...
        // User's STANDARD UTC offset (DST-stripped) → zone-grid character.
        // Highlight cells whose per-cell zone code matches. Real-world zones
        // zigzag along country borders, so column-based highlighting is wrong
        // for places like China (all of it on UTC+8 despite spanning 5
        // longitudinal column-widths) or US Central Time (Texas/Houston are
        // at Mountain-Time longitude but use Central Time).
        let currentOffset = TimeZone.current.secondsFromGMT()
        let dstOffset = Int(TimeZone.current.daylightSavingTimeOffset())
        let standardOffset = currentOffset - dstOffset
        let userZoneChar = Self.zoneGridChar(forStandardOffsetSeconds: standardOffset)

        // Column-band fallback (used if the offset doesn't map to a zone char,
        // e.g., obscure half-hour offsets we haven't placed in the grid).
        let zoneOffsetHours = Double(standardOffset) / 3600.0
        ...
        let mapPaths = Self.continentDotPaths(
            ...
            userZoneChar: userZoneChar,
            columnFallbackRange: zoneStart...zoneEnd
        )
    ...
    /// Per-cell time-zone offset code (matches `mapGrid` layout).
    /// Encoding: '0'=UTC+0, '1'..'9'=UTC+1..+9, 'A'..'E'=UTC+10..+14,
    /// 'a'..'l'=UTC-1..-12, '*'=UTC+5:30 (India), '^'=UTC-3:30 (Newfoundland),
    /// '?'=unspecified, '.'=water.
    private static let zoneGrid: [String] = [
        "................................................................................................",
        "............................cccc................................................................",
        "....iiiiiiii...............cccccc...............333333355555555666666677777788888899999AAACC....",
        "...iiiiiiiiiii.............ccccccc.............2333333355555555666666677777788888899999AAACC....",
        "....iiiiiiiiiihhhgggggffffeeeecccc..........00.1111112333555555566666677777788888899999AAAA.....",
        "..jj.iiiiiiiiihhhhgggggffffeeeeddd..00......000.1112222..3334888888888888888888888888...........",
        "..............hhhgggggffffeeeeecddd^........00.111111222....888888888888888888888888.99.........",
        "..............hhhgggggffffeeeee....^.........01111111222233348888888888888888888889.99..........",
        "..............hhhggggfffffeeeee...............11111112222333448888888888888888888.9.99..........",
        "..............hhhgggffffffeeeee...............1111111222.33345******6688888888888...99..........",
        "..............hhhgggfffffeeeee................0000011111233335******66888888888.................",
        "..............hhh..ffffff.eee.................00000111112.3335*****6667777.8888.................",
        "..................ffffff...ee.................00000111112.....****6666777778888.................",
        ".....jjj..........fffff..ee.dd.................0000111112......***666677777......88.............",
        "...................ffff.....dd...................111113333......**.....7777......88.............",
        "...................fff............................11113333.......*......777.......8.............",
        "....................ff............................1111333..............777788888................",
        ".....................feeedddccccc.................111122.................7788888................",
        "......................eeeccccccccc................111122...................88889999999..........",
        "......................eeecccccccc..................11122..3.....................999999999.......",
        "......................eedccccccc....................2222..33......................9999999.......",
        ".......................dccccccc.....................2222..3......................8888999AAAAA...",
        ".......................dcccccc......................222.........................88888999AAAAA...",
        ".......................dccccc........................22..........................8888999AAAA....",
        "........................cccc.........................2............................888999AAA.....",
        "........................ccc........................................................AAAAA........",
        "........................cc............................................................AA........",
        "........................c.......................................................................",
    ...
    /// Maps the user's STANDARD (non-DST) UTC offset in seconds to the
    /// zone-grid character. Returns `nil` for offsets that don't have a
    /// representative cell in the grid (rare zones like UTC+13).
    private static func zoneGridChar(forStandardOffsetSeconds offsetSec: Int) -> Character? {
        let absMins = abs(offsetSec) / 60
        let hours = abs(offsetSec) / 3600
        let minsRemainder = absMins % 60

        // Special half-hour zones first
        if offsetSec == 5 * 3600 + 30 * 60 { return "*" }  // India
        if offsetSec == -(3 * 3600 + 30 * 60) { return "^" }  // Newfoundland

        // Whole-hour zones — drop sub-hour remainders
        guard minsRemainder == 0 else { return nil }

        if offsetSec == 0 { return "0" }
        if offsetSec > 0 {
            switch hours {
            case 1...9: return Character(String(hours))
            case 10: return "A"
            case 11: return "B"
            case 12: return "C"
            case 13: return "D"
            ...
            // -1..-12 → 'a'..'l'
            guard hours >= 1 && hours <= 12 else { return nil }
            let asciiOffset = hours - 1
            let scalar = Unicode.Scalar(Character("a").asciiValue! + UInt8(asciiOffset))
            return Character(scalar)
        }
    }

    /// Builds two CGPaths from `mapGrid` + `zoneGrid`: light-gray dots for
    /// land cells OUTSIDE the user's current time zone, dark dots for land
    /// cells INSIDE it. Matches by per-cell zone character (so US Central
    /// Time highlights Texas/Mexico/Chicago — not a straight vertical column).
    /// Falls back to column-band highlighting if the user's offset doesn't
    /// map to a grid character.
    private static func continentDotPaths(
        size: CGSize,
        userZoneChar: Character?,
        columnFallbackRange: ClosedRange<Int>
        ...
        for (rowIdx, mapRow) in mapGrid.enumerated() {
            // Row 0 is north (top); CALayer y is up, so flip.
            let yIdx = mapRows - 1 - rowIdx
            let cy = (CGFloat(yIdx) + 0.5) * cellH
            let zoneRow = zoneGrid[rowIdx]
            let mapChars = Array(mapRow)
            let zoneChars = Array(zoneRow)
            for colIdx in 0..<mapCols where mapChars[colIdx] == "#" {
                let cx = (CGFloat(colIdx) + 0.5) * cellW
                let rect = CGRect(
                    x: cx - dotR, y: cy - dotR,
                    width: dotR * 2, height: dotR * 2
                )
                let cellZoneChar = zoneChars[colIdx]
                let isHighlighted: Bool
                if let userZone = userZoneChar {
                    isHighlighted = cellZoneChar == userZone
                } else {
                    isHighlighted = columnFallbackRange.contains(colIdx)
                }
                if isHighlighted {
